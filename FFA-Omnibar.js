    ...
    const SuggestModule = {
        ...
        _cleanupCache() {
            ...
            for (const [key, item] of entries) {
                if (now - item.ts > this._CACHE_TTL) {
                    this._cache.delete(key);
                    cleanedCount++;
                }
            }
            // 预分配空间以减少 Map 扩容
            if (cleanedCount > 0 && this._cache.size < this._CACHE_MAX) {
                this._cache = new Map(this._cache);
            }
        },
    ...
    function init() {
        ...
        function renderToolbar() {
            ...
            input.onfocus = () => {
                ...
                // 获取当前选中的引擎URL
                const selectedBtn = toolbar.querySelector('.engine-btn.active');
                const engineUrl = selectedBtn ? selectedBtn.dataset.engineUrl : enabled[0]?.url;
                if (engineUrl) {
                    fetchSuggestions(input.value, suggestBox, mask, engineUrl);
                }
            };
            input.onblur = () => {
